...
Earlier releases might work, but aren't tested
- [mypy >= v0.920](https://pypi.org/project/mypy) or [pyright >= 1.1.171](https://github.com/microsoft/pyright)
- [python-protobuf >= 3.19.1](https://pypi.org/project/protobuf/) - matching protoc release
...
Earlier releases might work, but aren't tested
- [grpcio>=1.43.0](https://pypi.org/project/grpcio/)
- [grpcio-tools>=1.43.0](https://pypi.org/project/grpcio-tools/)
...
If you are using proto3, then primitives cannot be represented as NULL on the wire - only as their
zero value - unless declared explicit presence with the
[`optional` keyword](https://github.com/protocolbuffers/protobuf/blob/master/docs/field_presence.md#how-to-enable-explicit-presence-in-proto3).
By default mypy-protobuf types message constructors to have non-nullable primitives (eg `int`
instead of `Optional[int]`). python-protobuf itself will internally convert None -> zero value. If
you intentionally want to use this behavior, set this flag! We recommend avoiding this, as it can
lead to developer error - confusing NULL and 0 as distinct on the wire.  However, it may be helpful
when migrating existing proto2 code, where the distinction is meaningful:
...
- [@icgood](https://github.com/icgood)
